...
(function () {
  ...
  const SESSION_SELECT_TIMEOUT_MS = 1500;
  ...
  const SESSION_MODE_DEFAULTS = [
    { label: "Normal", keyword: "normal", aliases: ["normal"] },
    { label: "Prioridade", keyword: "prioridade", aliases: ["prioridade", "prioritario", "priority"] },
  ];
  const PRIORITY_OVERRIDE_BOSS_NAMES = [
    "First Adept",
    "Pain Invoker",
    "Lost Spirit",
    "Heart Keeper",
  ...
  const PRIORITY_OVERRIDE_ELDER_KEYWORDS = new Set(["elder", "elder2"]);
  ...
  function validateConfig(config) {
    ...
    const sessionMode = normalize(config.sessionMode || "normal");
    ...
    if (!SESSION_MODE_DEFAULTS.some((item) => item.keyword === sessionMode)) {
      return null;
    }
  ...
  function chooseSessionMode(rawValue) {
    const raw = String(rawValue || "").trim();
    if (/^\d+$/.test(raw)) {
      const index = Number(raw) - 1;
      if (SESSION_MODE_DEFAULTS[index]) {
        return SESSION_MODE_DEFAULTS[index];
      }
    }

    const normalized = normalize(raw);
    return (
      SESSION_MODE_DEFAULTS.find((item) =>
        [item.label, item.keyword, ...(item.aliases || [])].some(
  ...
  function promptConfig(forcePrompt = false) {
    ...
    const sessionHelp = SESSION_MODE_DEFAULTS.map(
      (item, index) => `${index + 1}=${item.label}`
    ).join("\n");
    const currentSessionMode =
      SESSION_MODE_DEFAULTS.find((item) => item.keyword === saved?.sessionMode)?.label ||
      "Normal";
    ...
    const sessionMode = chooseSessionMode(sessionInput);
    if (!sessionMode) {
      alert("Tipo de sessao invalido.");
      return null;
    }
    ...
    const config = {
      ...
      keyword,
      sessionMode: sessionMode.keyword,
    };
    saveConfig(config);
    return config;
  }
  ...
  function shouldForcePriorityMode(anchor) {
    const bossNameToken = normalize(anchor?.textContent || "");
    const contextToken = normalize(getContextText(anchor));
    return PRIORITY_OVERRIDE_BOSS_NAMES.some((bossName) => {
      const token = normalize(bossName);
      return bossNameToken.includes(token) || contextToken.includes(token);
    });
  }

  function resolveSessionModeForBoss(config, anchor) {
    if (
      PRIORITY_OVERRIDE_ELDER_KEYWORDS.has(String(config?.keyword || "").toLowerCase()) &&
      shouldForcePriorityMode(anchor)
    ) {
      return "prioridade";
    }
  ...
  function findSessionModeSelect() {
    const selectCandidates = [];
    for (const root of getActionSearchRoots()) {
      selectCandidates.push(...Array.from(root.querySelectorAll("select")));
    }

    return (
      selectCandidates.find((select) => {
        if (!isVisible(select)) {
          return false;
        }
        ...
        return text.includes("tipodesessao") || text.includes("normal") || text.includes("prioridade");
      }) || null
    );
  }

  function findSessionModeOption(select, sessionModeKeyword) {
    const sessionMode =
      SESSION_MODE_DEFAULTS.find((item) => item.keyword === sessionModeKeyword) ||
      SESSION_MODE_DEFAULTS[0];
    const aliases = [sessionMode.label, sessionMode.keyword, ...(sessionMode.aliases || [])].map(normalize);
  ...
  function applySessionModeToSelect(select, option) {
    if (!select || !option) {
      return false;
    }
  ...
  async function ensureSessionModeApplied(sessionModeKeyword) {
    if (state.sessionModeApplied) {
      return true;
    }

    const deadline = Date.now() + SESSION_SELECT_TIMEOUT_MS;
    while (Date.now() <= deadline) {
      const select = findSessionModeSelect();
      if (select) {
        const option = findSessionModeOption(select, sessionModeKeyword);
        if (option) {
          applySessionModeToSelect(select, option);
          state.sessionModeApplied = true;
          return true;
        }
      }
  ...
  async function run() {
    ...
    const tryBossSequence = async (bossAnchor) => {
      ...
      state.sessionModeApplied = false;
      const effectiveSessionMode = resolveSessionModeForBoss(config, bossAnchor);
      ...
      await ensureSessionModeApplied(effectiveSessionMode);
