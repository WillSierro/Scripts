// ==UserScript==
// @name         Lamentosa Boss Sem
// @namespace    codex.lamentosa
// @version      1.0.0
// @description  Monitora o chat do boss, abre qualquer boss fora da lista, clica em Desafiar e depois no OK.
// @match        *://*/*
...
(function () {
  ...
  const OPEN_LISTED_BOSSES_ONLY = false;
  ...
  const STORAGE_KEY = "lamentosaBossSemConfig";
  const ENABLED_STORAGE_KEY = "lamentosaBossSemEnabled";
  const CONTROL_BUTTON_ID = "lamentosa-boss-sem-config-btn";
  const UI_STACK_ID = "lamentosa-ui-stack";
  const UI_SLOT_ID = "lamentosa-boss-sem-slot";
  const UI_ORDER = 17;
  const BUTTON_ACTIVE_BG = "#2f8f46";
  const BUTTON_INACTIVE_BG = "#9a2f2f";
  const DEFAULT_BUTTON_TITLE =
    "Clique esquerdo configura e liga. Clique direito liga/desliga. Ctrl+Alt+S reconfigura.";
  ...
  function log(message) {
    console.log(`[Lamentosa Boss Sem] ${message}`);
    showToast(message);
  }
  ...
  function activateElement(element) {
    ...
    if ((tagName === "button" || tagName === "input") && type === "submit" && element.form) {
      try {
        ...
        console.warn("[Lamentosa Boss Sem] requestSubmit falhou, tentando click normal", error);
      }
    }
    ...
    if (!forcePrompt && saved) {
      log(
        `Usando configuracao salva: ${saved.categoryLabel} / ${saved.keyword} / ${saved.time}. ` +
          "Use o botao Boss Sem ou Ctrl+Alt+S para reconfigurar."
      );
      return saved;
    }
    ...
    const observer = new MutationObserver(async (mutations) => {
      ...
      if (liveBoss) {
        ...
        if (!success && finalMessage) {
          log(`Boss Sem finalizado: ${finalMessage}`);
        }
        return;
      }
  ...
  run().catch((error) => {
    console.error("[Lamentosa Boss Sem] erro inesperado", error);
